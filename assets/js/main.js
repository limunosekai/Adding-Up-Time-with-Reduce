const timeNodes = Array.from(document.querySelectorAll('[data-time]')); //data값이 time인 nodelist를 array로 바꿔서 추가하기

const seconds = timeNodes
    .map(node => node.dataset.time) //li에서 time만 갖고 오기
    .map(timeCode => {
      const [mins, secs] = timeCode.split(':').map(parseFloat); //split으로 ':'를 기준으로 분,초를 나눈 후 문자열 -> 실수화
